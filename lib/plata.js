...
class Plata {
	...
	async createReceipt(invoiceId, email) {
		try {
			if (!invoiceId) return { result: 0, error: "invoiceId field is required" };
	...
	async getFiscalChecks(invoiceId) {
		try {
			...
			const plataToken = await this.getPlataToken();
			const response = await axios.get(
				`${PLATA_API_URL}/merchant/invoice/fiscal-checks?invoiceId=${invoiceId}&email=${email}`,
				{
					headers: {
						"X-Token": plataToken,
					},
			...
			if (data.status === "success") {
				...
				if (
					...
					if (order.email === "artemgergetc@gmail.com") {
						const checks = this.getFiscalChecks(invoiceId);
