    ...
    try {
      ...
      if (data.status === "success") {
        ...
        if (order.salesOrderID && order.productPrice && order.currency) {
          ...
          if (order.email === "artemgergetc@gmail.com") {
            const { checks } = await this.getFiscalChecks(invoiceId);
            const finishedChecks = checks.filter(
              (check) => check.status === "done" && check.file
            );
            const checksFiles = finishedChecks.map((check) => check.file);

            if (finishedChecks.length) {
              await sendTransactionalEmail(
                "Ваш фіскальний чек",
                "Ваш фіскальний чек у вкладені",
                order.email,
                finishedChecks.map((check, checkIndex) => ({
                  fileName: `Фіскальний чек - ${checkIndex + 1}`,
            ...
            plataLogger.info("fiscal checks: ", checks);
            plataLogger.info("fiscal checks files: ", checksFiles);
          }
        }
      }
