...
class Plata {
  ...
  async processCallback(data) {
    try {
      ...
      let order = await PlataOrder.findOne({ invoiceId });
      if (!order) {
        return {
          result: 0,
      ...
      if (data.status === "success") {
        ...
        if (order.salesOrderID && order.productPrice && order.currency) {
          ...
          order = await PlataOrder.findOne({ invoiceId });
          if (!order.isZohoReceiptSent) await zoho.createPayment(zohoPayload);
          order.isZohoReceiptSent = true;
          await order.save();
