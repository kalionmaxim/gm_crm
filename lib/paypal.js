		...
		try {
			...
			const createPaymentJSON = {
				...
				"redirect_urls": {
					...
					"cancel_url": data.redirectURL || (config.get("url") + "checkout/3?productName=" + encodeURIComponent(data.productName)).toString()
				},
				"transactions" : [{
					"item_list"  : {
						"items": [{
						...
						if (link) {
							if (payment.id) {
								const paypalPayment = new PaypalPayment({
									...
									successURL  : data.redirectURL || (config.get("url") + "checkout/3?productName=" + encodeURIComponent(data.productName)).toString(),
									failureURL  : data.redirectURL || (config.get("url") + "checkout/3?productName=" + encodeURIComponent(data.productName)).toString()
								});
