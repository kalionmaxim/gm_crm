...
const URL = config.get("monobank_test:url");
const STORE_ID = config.get("monobank_test:store_id");
const STORE_SECRET = config.get("monobank_test:store_secret");
		...
		try {
			...
			if (order) {
				...
				if ((data.state === "IN_PROCESS") && (data.order_sub_state === "WAITING_FOR_STORE_CONFIRM")) {
					...
					// const stateData = await this.stateOrder(data.order_id);
					// console.log("stateData:", stateData);
					// console.log("stateData.data:", stateData.data);
					...
					if (respData.result) {
						...
						if (respData.data.state === "SUCCESS") {
							...
							const data = {
								...
								total_sum    : (order.total_sum / 100).toFixed(2),
								Currency     : "USD"
							};
...
module.exports = new MonobankTest();
