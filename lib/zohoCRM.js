...
class Zoho {
	...
	async searchContactAndUpdateOrAddNew(data, geoData) {
		try {
			...
			if (contactID) {
				...
				const createContactResult = await this.createContactTilda(data, geoData);
	...
	async createContactTilda(data, geoData) {
		try {
			let referrer = "";
			if (data.http_refferer) {
				if (data.http_refferer.length >= 255) {
			...
			const jsonData = {
				...
				// "academy_date" : new Date().toISOStringTimezoneOffset() + ""
			};
			...
			if (result) {
				...
				crmLogger.info("createContactTilda() – response from zoho is incorrect");
				return {
					result: 0,
					error : "Response from zoho is incorrect"
				};
			...
			crmLogger.error(`createContactTilda() – ${err}`);
			return {
				result: 0,
				error : "Unknown error"
			};
