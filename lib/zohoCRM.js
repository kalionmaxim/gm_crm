...
class Zoho {
  ...
  try {
    ...
    if (data.contactID) {
      ...
      contactID = await Zoho.getOrCreateContact(data);
    }
    ...
    const search = await Zoho.searchDeal(contactID, productId);

    // Якщо угода вже існує — додаємо нотатку і повертаємо ID
    if (search?.id) {
      const dealID = search.id.toString();

      if (data.status || data.model || data.niche || data.target || data.result) {
        try {
          const noteContent = [
            `Статус: ${data.status || ""}`,
            `Модель: ${data.model || ""}`,
          ...
          const noteData = {
            Note_Title: "Дані з форми",
            Note_Content: noteContent,
            Parent_Id: dealID,
            se_module: "Deals"
          };

          await ZohoHelpers.createRecord("Notes", noteData, "approval, workflow, blueprint");
        } catch (noteErr) {
          crmLogger.error(`createDeal() – failed to create note for existing deal ${dealID}: ${noteErr}`);
        }
      }
    ...
    // Якщо угода не знайдена — створюємо нову
    const jsonData = {
      Contact_Name: contactID,
      Deal_Name: `${data.name} - ${data.product?.name ?? "Новий продукт"}`,
      Stage: "New",
      ...
      Custom_telephone: data.custom_telephone,
    ...
    if (result) {
      const dealID = result.data[0].details.id.toString();
      ...
      if (data.status || data.model || data.niche || data.target || data.result) {
        try {
          ...
          crmLogger.error(`createDeal() – failed to create note for new deal ${dealID}: ${noteErr}`);
        }
      }
    ...
    crmLogger.error("createDeal() error", e);
    return {
      result: 0,
      error: e.toString()
    };
