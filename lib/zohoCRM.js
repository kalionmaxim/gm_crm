...
// const iLogger = require("./logger").iLogger;

// const Plan = require("../models/plan").Plan;
// const AccessConsultation = require("../models/accessConsultation").AccessConsultation;

const ZohoHelpers = require("./zohoHelpers");

class Zoho {
	// Передаем plan, если это оплата подписки, а productName и productID опускаем
	// Или оставляем plan как null, но передаем productName и productID, если это оплата консультаций, видео, архива
	/*async createInvoice(user, order, plan, productName, productID, description) {
		try {
			let pName = null;
			let pID = null;
			if (plan && plan.productID) {
				pName = plan.productName;
				pID = plan.productID;
			} else {
				pName = productName;
				pID = productID;
			...
			if (pName && pID) {
				...
				let potentialID = user.potentialID;

				if (!contactID) {
					contactID = await this.searchContactOrAddNew(user);
				}

				if (!potentialID) {
					potentialID = await this.createDeal(user, order, plan, productName, productID);
				}

				if (contactID && potentialID) {
					const jsonData = {
						"Subject"        : pName + "- " + user.firstName,
						"Contact_Name"   : contactID,
						"Deal_Name"      : potentialID,
						"vystav_sum"     : order.amount / 100,
						"oplach_sum"     : order.amount / 100,
						"Status"         : "Оплачен",
						...
						"Product_Details": [{
							...
							"quantity"            : 1,
							"total_after_discount": order.amount / 100,
							"net_total"           : order.amount / 100,
							"list_price"          : order.amount / 100,
					...
					if (result) {
						const invoiceID = result.data[0].details.id;
						...
						crmLogger.info("createInvoice() – response from zoho is incorrect");
						return null;
					}
				} else {
					crmLogger.info("createInvoice() – contactID and/or potentialID are undefined");
					return null;
				}
			} else {
				crmLogger.info("createInvoice() – productName and/or productID are undefined");
				return null;
			}
		} catch (err) {
			crmLogger.error(`createInvoice() – ${err}`);
			return null;
		}
	}*/

	/*async changePotentialStatus(user, status) {
		try {
			let potentialID = user.potentialID;

			if (potentialID) {
				const jsonData = {
					"status_podpiski": status
				};

				const result = await ZohoHelpers.updateRecord("Potentials", potentialID, jsonData);

				if (result) {
					return potentialID;
				} else {
					crmLogger.info("changePotentialStatus() – response from zoho is incorrect");
					return null;
				}
			} else {
				crmLogger.info("changePotentialStatus() – potentialID is undefined");
				return null;
			}
		} catch (err) {
			crmLogger.error(`changePotentialStatus() – ${err}`);
			return null;
		}
	}*/
		...
		try {
			...
			if (contactID) {
				if (data.productID) {
					...
					let deals = [];
					if ((typeof (dealsData.data) === "object") && (dealsData.data instanceof Array) && (dealsData.data.length > 0)) {
