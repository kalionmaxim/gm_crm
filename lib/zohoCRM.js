		...
		try {
			...
			if (contactID) {
				const jsonData = {
					...
					"http_refferer": data.http_refferer || "",
					"Amount"       : data.Amount || 0,
					"Country"      : (geoData && geoData.country_name) || "",
					"City"         : (geoData && geoData.city) || "",
					"Time_zone"    : (geoData && ("UTC" + geoData.utc_offset)) || ""
				};
	...
	async createContact(data, geoData) {
		try {
			const jsonData = {
				...
				"http_refferer": data.http_refferer || "",
				"URL"          : data.url || "",
				"field2"       : (geoData && geoData.country_name) || "",
				"field4"       : (geoData && geoData.city) || "",
				"field5"       : (geoData && ("UTC" + geoData.utc_offset)) || ""
			};
