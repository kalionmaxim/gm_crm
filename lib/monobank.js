...
const INSTALLMENT_COMMISSIONS = [
  2.9, 4.1, 5.9, 7.2, 8.3, 9.5, 10.8, 12, 13.2, 14.3, 15.5, 16.6, 17.7, 18.8,
  19.8, 20.9, 21.9, 23, 24, 24.9, 25.9, 26.8, 27.8,
]; // in percent; starts from payment for 3 months
...
class Monobank {
	...
	async calculateCommision(order) {
		try {
			const rate = (parseFloat((await USDRate.findOne({ currency: "UAH" }).lean()).price).toFixed(2)) || 0;
			if (!rate) return;
			let commisionSum;
			if (order.available_programs && order.available_programs[0] && order.available_programs[0].available_parts_count && order.available_programs[0].available_parts_count[0]) {
				const parts = order.available_programs[0].available_parts_count[0];
				if (!INSTALLMENT_COMMISSIONS[parts - 3]) return;
				commisionSum = (order.total_sum * (INSTALLMENT_COMMISSIONS[parts - 3] / 100) / rate).toFixed(2);
			}
			return commisionSum;
		} catch (error) {
			console.error(error);
	...
	async calculateSalarySum(totalSumUah, commisionUsd) {
		try {
			if (!totalSumUah || !commisionUsd) return;
			const rate = (parseFloat((await USDRate.findOne({ currency: "UAH" }).lean()).price).toFixed(2)) || 0;
			if (!rate) return;
			return ((totalSumUah / rate) - commisionUsd).toFixed(2);
		} catch (error) {
			console.error(error);
		}
	}
				...
				if ((data.state === "IN_PROCESS") && (data.order_sub_state === "WAITING_FOR_STORE_CONFIRM")) {
					...
					if (respData.result) {
						...
						if (respData.data.state === "SUCCESS") {
							...
							const commisionSum = await this.calculateCommision(order);
							...
							const data = {
								...
								commisionSum,
								salarySum: await this.calculateSalarySum(order.total_sum, commisionSum)
							};
