			...
			if (orders && orders.length > 0) {
				for (const order of orders) {
					...
					if (requestCallbackResult && requestCallbackResult.data) {
						...
						order.paymentState = requestCallbackResult.data && requestCallbackResult.data.paymentState ? requestCallbackResult.data.paymentState : requestCallbackResult.data;
	...
	async processCallback (body) {
		try {
			if (body) {
				...
				if (order) {
					order.paymentState = body && body.paymentState ? body.paymentState : body;
					order.status = "processed";
					await order.save();
					await this.sendDataToCrm(order);
				}
