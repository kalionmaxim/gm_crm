...
class PrivatBank {
	...
	async requestCallbacks () {
		try {
			const orders = await PrivatOrder.find({ status: "not_processed" });
			if (orders && orders.length > 0) {
				for (const order of orders) {
					await this.wait(1000);
					const requestCallbackResult = await axios.post(`${API_URL}payment/callback`, { orderId: order.internal_order_id });
					if (requestCallbackResult && requestCallbackResult.data) {
						iLogger.info(`Privatbank request callback result: ${order.internal_order_id} | ${JSON.stringify(requestCallbackResult.data)}`);
						order.invoice.paymentState = requestCallbackResult.data && requestCallbackResult.data.paymentState ? requestCallbackResult.data.paymentState : requestCallbackResult.data;
	...
	async sendDataToCrm (order) {
		try {
			if (order) {
				const data = {
					invoice: JSON.stringify(order.invoice),
	...
	async processCallback (body) {
		try {
			if (body) {
				...
				if (order) {
					order.invoice.paymentState = body && body.paymentState ? body.paymentState : body;
					order.status = "processed";
					await order.save();
					await this.sendDataToCrm(order);
				}
