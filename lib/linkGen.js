	...
	try {
		...
		if (ctx.request && ctx.request.query) {
			...
			if (productName && productID && productPrice && currency && merchantID) {
				return {
					status: 200,
					link: `${baseUrl}checkout/1?productID=${productID}&productName=${productName}&productPrice=${productPrice}&currency=${currency}&merchantID=${merchantID}&lang=ru&payPalHide=true&fondyHide=true`
				};
			} else {
				return {
					status: 400
