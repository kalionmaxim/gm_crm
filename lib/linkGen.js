		...
		if (ctx.request && ctx.request.query) {
			...
			if (productName && productID && productPrice && currency && merchantID) {
				...
				case "mono":
					...
					break;
				case "frisbee":
					link = encodeURI(`${baseUrl}checkout/1?productID=${productID}&productName=${productName}&productPrice=${productPrice}&currency=${currency}&merchantID=${merchantID}&lang=ru&payPalHide=true&fondyHide=true&privatHide=true&monoHide=true&tinkoffHide=true`);
					break;
				case "privat":
					link = encodeURI(`${baseUrl}checkout/1?productID=${productID}&productName=${productName}&productPrice=${productPrice}&currency=${currency}&merchantID=${merchantID}&lang=ru&payPalHide=true&fondyHide=true&monoHide=true&frisbeeHide=true&tinkoffHide=true`);
					break;
				}
