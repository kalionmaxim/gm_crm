...
class WayForPay {
	...
	async processOrder(data) {
		try {
			if (data) {
				console.log(`WayForPay callback body: ${data}`);
				iLogger.info(`WayForPay callback body: ${JSON.stringify(data)}`);
				...
				iLogger.info(`WayForPay callback body (no data): ${JSON.stringify(data)}`);
				return {
					result: 0
				};
			}
