...
class ZohoService {
	async oAuth() {
		try {
			...
			if (accessToken && (accessExpiresDate > now)) {
				...
				return new Promise(resolve => {
					throttledRequest(options, function (error, response) {
						if (error) {
							eLogger.error(`ZohoHelpers oAuth request error: ${error}`);
						}
						...
						if (body && body.access_token && body.expires_in) {
							...
							accessExpiresDate.setSeconds(accessExpiresDate.getSeconds() + parseInt(body.expires_in, 10) - 100);
							resolve(accessToken);
						} else {
							resolve(null);
						}
			...
			eLogger.error(`ZohoHelpers oAuth error: ${err}`);
			return null;
		}
	}
	...
	async getDataFromCrm(url, params, method) {
		try {
			...
			if (oAuthData) {
				...
				return new Promise(resolve => {
					crmLogger.info("getDataFromCrm() request sent:", method, url, (params || {}));
					throttledRequest(options, (error, response) => {
						if (error) {
							eLogger.error(`ZohoHelpers getDataFromCrm request error: ${error}`);
							resolve(null);
			...
			eLogger.error(`ZohoHelpers getDataFromCrm error: ${new Error(err)}`);
			return null;
		}
	}
	...
	async searchByCriteria(moduleName, criteria) {
		try {
			...
			eLogger.error(`ZohoHelpers searchByCriteria error: ${new Error(err)}`);
			return null;
		}
	}
	...
	async searchByCriteriaCOQL(criteria) {
		try {
			...
			eLogger.error(`ZohoHelpers searchByCriteriaCOQL error: ${new Error(err)}`);
			return null;
		}
	}
	...
	async createRecord(moduleName, jsonData, wfTriger) {
		try {
			...
			eLogger.error(`ZohoHelpers createRecord error: ${new Error(err)}`);
			return null;
		}
	}
	...
	async getDeals(contactID) {
		try {
			...
			eLogger.error(`ZohoHelpers getDeals error: ${new Error(err)}`);
			return null;
		}
	}
	...
	async getRecord(moduleName, id) {
		try {
			...
			eLogger.error(`ZohoHelpers getRecord error: ${new Error(err)}`);
			return null;
		}
	}
	...
	async updateFieldValue(moduleName, id, valueApiName, value) {
		try {
			...
			eLogger.error(`ZohoHelpers updateFieldValue error: ${new Error(err)}`);
			return null;
		}
	}
	...
	async updateRecord(moduleName, id, jsonData, wfTriger) {
		try {
			...
			eLogger.error(`ZohoHelpers updateRecord error: ${new Error(err)}`);
			return null;
		}
	}
}
