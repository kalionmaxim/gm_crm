...
function getListOfCustomFields (name, callback) {
	if (typeof name === "string" && name !== "") {
		...
		axios.get(apiUrl + "/custom-fields", axiosConfig).then(function (r) {
			getResponseLogger.info("[" + new Date() + "]: received custom fields - " + ((r && r.data) ? r.data.length : "null"));
			if (r && r.data) {
				if (r.data && r.data.length > 0) {
