			...
			if (data.merchantID && (data.merchantID.length > 0)) {
				...
				if (merchant) {
					if (data.salesOrderID) {
						...
						console.log("Fondy order_id is:", "GeniusMarketingOrder-" + (parseInt(merchant.lastOrderID, 10) + 1));
						...
						const requestToFondy = {
							...
							order_id           : "GeniusMarketingOrder-test-order-3-" + (parseInt(merchant.lastOrderID, 10) + 1),
		...
		try {
			...
			if (data.merchantID && (data.merchantID.length > 0)) {
				...
				if (merchant) {
					...
					const url = "https://api.fondy.eu/api/recurring";
					const options = {
						method: "post",
						body  : rSend,
						json  : true,
